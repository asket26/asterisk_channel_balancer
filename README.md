...
Для отключения канала достаточно установить его current-state ненулевым ( небольшой баг- в этот момент не должно быть звонков в этом канале, иначе по окончании звонка он опять будет помечен как свободный).
